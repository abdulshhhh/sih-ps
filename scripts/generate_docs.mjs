...
// Self-contained runner using the PDF Builder logic to create all 15 PDFs
class SimplePdfBuilder {
  ...
  build(def) {
    ...
    streamCommands.push(`0.85 0.90 0.95 rg 0 832 595 10 re f`);
    streamCommands.push(`0.10 0.22 0.38 RG 2 w`);
    ...
    const streamObj = this.addObject(`<< /Length ${streamLength} >>\nstream\n${streamContent}\nendstream`);
...
const publicDir = path.join(process.cwd(), 'public', 'mock-documents');
...
for (const [filename, def] of Object.entries(DOC_DEFINITIONS)) {
  const buf = builder.build(def);
  fs.writeFileSync(path.join(publicDir, filename), buf);
  fs.writeFileSync(path.join(uploadsDir, filename), buf);
  count++;
}

console.log(`Successfully generated ${count} valid synthetic PDF documents into public/mock-documents/ and .uploads/`);
